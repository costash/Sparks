...
int read_State(State* state)
{
  ...
  /* Read in input type. */
  while(scanf_s("%s", inputType, 256)) {
    if(strcmp(inputType, "end") == 0) {
      state->gameOver = true;
      break;
    } else if(strcmp(inputType, "turn") == 0) {
      if (!scanf_s("%d", &(state->currentTurnNumber))) { return false; }
      break;
    } else {
  ...
  if(state->currentTurnNumber == 0) {
    ...
    while(scanf_s("%s", inputType, 256) == 1) {
      if(strcmp(inputType, "loadtime") == 0) {
        if (!scanf_s("%lf", &loadTime)) { return false; }
      } else if(strcmp(inputType, "turntime") == 0) { 
        if (!scanf_s("%lf", &turnTime)) { return false; }
      } else if(strcmp(inputType, "rows") == 0) {
        if (!scanf_s("%d", &mapRows)) { return false; }
      } else if(strcmp(inputType, "cols") == 0) {
        if (!scanf_s("%d", &mapColumns)) { return false; }
      } else if(strcmp(inputType, "turns") == 0) {
        if (!scanf_s("%d", &totalTurnsNumber)) { return false; }
      } else if(strcmp(inputType, "player_seed") == 0) {
        if (!scanf_s("%lld", &seed)) { return false; }
        srand((unsigned int)seed);
      } else if(strcmp(inputType, "viewradius2") == 0) {
        if (!scanf_s("%lf", &viewRadius)) { return false; }
      } else if(strcmp(inputType, "attackradius2") == 0) {
        if (!scanf_s("%lf", &attackRadius)) { return false; }
      } else if(strcmp(inputType, "spawnradius2") == 0) {
        if (!scanf_s("%lf", &spawnRadius)) { return false; }
    ...
    while(scanf_s("%s", inputType, 256) == 1) {
      if(strcmp(inputType, "w") == 0) {
        /* Water square. */
        if (!(scanf_s("%d%d", &row, &col) == 2)) { return false; }
        ...
        /* Live ant square. */
        if (!(scanf_s("%d%d%d", &row, &col, &player) == 3)) { return false; }
